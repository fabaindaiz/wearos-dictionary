...
`prompt-evaluate.md` — in `.agents/` or wherever it was dropped in this
repo.
...
Then do what the rest of the document says, reading only this:
...
Privacy (principle 20): the report never travels, and nothing from it enters
the bundle unless it is generalised so that no private repository, person or
user can be reconstructed from it.
...
The evaluation procedure: what to read, the nine questions, the scoring and the
report. Everything shared, and the precedence between documents, is in
`prompt-context.md` §*The set*.

**This prompt degrades honestly.** With `prompt-context.md` present it scores
against the sections its `Reads:` list names; without it, it still inventories,
still checks what is already false, still runs the gate and still writes the
report. **Say in the report which of the two you did**, because it changes what
the scores mean.
...
Write it to `.agents/evaluation-<YYYY-MM-DD>-<content6>.md`, or wherever the repository
