...
**There are now FOUR mirrors, not one**, and the last three are easy to forget because none of
them looks like normalisation:
...
| `payload.py` → the `TAG_*` **letters** | `PayloadCodec.kt` → its `TAG_*` letters | **a whole channel goes invisible**: the reader ignores a tag it does not know, so a letter that diverges throws the data away with no error, no log and a pack that opens fine. Pinned by `vectors/payload-fixture.tsv`, which Kotlin decodes byte for byte, and by `verify_pack.TAGS_CONOCIDOS`, which fails on a tag the builder wrote and nobody registered |
...
The fourth is pinned by a **fixture of the real index**
