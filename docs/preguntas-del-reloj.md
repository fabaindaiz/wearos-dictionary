...
| P-12 | Do the principal parts read well beside the headword at 234 dp? | Open `correr` in a rebuilt pack: `corriendo (gerundio) · corrido (participio)` should sit above the translations in a sense's body weight | **D-242**. ⚠️ **No longer blocked**: the packs of 2026-09-25 carry `F` --83.6 % of a sample of `es-core`-- and it was seen on the 384 dp emulator as *perros (plural) · perra (feminine)*. What is left is the width: 234 dp is 39 % narrower and that is where it competes with the definition |
| P-14 | An origin is carried **whole**, and the longest in the dump is **3,888 characters**. At 234 dp, how much of one is worth drawing before it stops being read? | Open `house` or `perro` and scroll past the senses to **Origen**; count the rows it takes and whether you reach the end of it or give up | **d-a2f271-bc2a3b**, built with no length cap against the recommendation of ~80 characters. The section is placed after the senses so a long one costs nothing to whoever does not want it, and a test pins that at 192 dp -- but *how many rows are too many* is a reading judgement and the emulator's 384 dp flatters it by 64 %. ⚠️ **The answer is a product decision with a price already measured**: a cap of 128 would take the five packs from +36.8 MB to +8.8, and the APK from +8.5 to +1.9 |
