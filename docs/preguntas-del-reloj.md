...
| P-14 | Does the keep-alive actually hold a session across a screen-off? | `python3 tools/watchsession.py start` has to print `wake lock tomado`; then `probe --seconds 600` with the watch left alone and its screen covered by hand. Every sample must read `alcanzable=SI` **and** `lock=si`, including the ones where `wakefulness` is `Dozing` | **Every other question here.** P-6 was not answered but *withdrawn*, and its reason was verbatim *«el debug del reloj es inestable para sesiones largas»*; P-3, P-4 and P-5 all need a session that outlives the screen. The mechanism is built and the gate is green, but the lock **has never been observed held**: the watch went offline before the APK could be installed |
