...
| P-15 | Does the keep-alive really stop itself when the session ends? | With it running, turn **wireless debugging** off on the watch and check `logcat -s WatchKeepAlive` for `Depuracion inalambrica apagada`; separately, with it running, turn **Wi-Fi** off and look for `Wi-Fi apagado`. In both cases `dumpsys power` must stop showing `watchkeepalive:adb` | Whether the safety net exists at all. The hour-long expiry is the floor; this is the part that reacts, and it **compiles and has never been seen firing**. If it does not work, a `stop` that fails to arrive leaves a wake lock held until the expiry, on a watch battery |
