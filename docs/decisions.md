...
| D-031 | El contenido viene de Wiktionary (CC BY-SA); la pantalla de atribución **no es opcional** | El DLE de la RAE no tiene licencia abierta ni API pública. Wiktionary es efectivamente la única fuente utilizable para definiciones en español | `SearchViewModelTest.laAtribucionYLaLicenciaSalenDelPackYNoDelCodigo` — **parcial**: fija que el estado lleva la atribución **del pack** y no una constante; que la pantalla la dibuje sigue sin enforcer, y eso necesita un test de UI instrumentado |
