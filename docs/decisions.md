...
| D-071 | ~~El pack viaja como asset del APK~~ **Revertida el 2026-09-17.** El APK no lleva ningún diccionario; los packs viven sólo en `filesDir/packs/` | La desviación decía que se revertía al existir el instalador. Lo que la adelantó fue **medir el inglés: 295,1 MiB en disco, 184,7 MiB comprimido**. Con los dos idiomas el APK se iba a ~270 MB, y por Bluetooth a un reloj eso no es un detalle. Hoy los packs entran por `adb push`; mañana el instalador escribe en el mismo directorio y la app no nota la diferencia | `PackStore` tiene un solo origen; `check_app_logic_is_jvm_testable` vigila la frontera |
